...
class PlusInstruction(Instruction):
    ...
    def execute(self, intcode, arguments):
        intcode.set(arguments[2].address,
...
class LessThanInstruction(Instruction):
    ...
    def execute(self, intcode, arguments):
        intcode.set(arguments[2].address, int(
            arguments[0].value < arguments[1].value))
        return self.new_pc(intcode)
    ...
    def _get_arguments(self, argument_num):
        ...
        for i in range(argument_num):
            ...
            if modes[i] == '1':
                ...
                arguments.append(Argument(self.program[value], value))
        return arguments

    def run(self):
        self.output = None
        while not self.halted and self.output is None:
            instruction = self._get_intruction(self.program[self.pc] % 100)
            arguments = self._get_arguments(instruction.argument_num)
